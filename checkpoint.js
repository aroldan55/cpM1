...
var isAncestor = function(genealogyTree, ancestor, descendant){
  // Tu código aca:
  if (!genealogyTree[ancestor].length) return false;
  for (let i=0; i < genealogyTree[ancestor].length; i++){
    let captureAncestor = genealogyTree[ancestor][i];
    if (captureAncestor === descendant) return true;
    if (genealogyTree[captureAncestor].length > 0) isAncestor(genealogyTree, captureAncestor, descendant);
    return false;
}
